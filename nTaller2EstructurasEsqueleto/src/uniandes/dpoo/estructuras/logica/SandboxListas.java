...
public class SandboxListas
{
    ...
    public List<Integer> getCopiaEnteros( ) {
    	List<Integer> copiaListaEnteros = new ArrayList<>();
    	
    	for (Integer entero :listaEnteros) {
    		
    		copiaListaEnteros.add(entero);
    	}
    ...
    {
    	List<String> copiaListaCadenas =  new LinkedList<>();
    	
    	for (String cadena:listaCadenas) {
    		copiaListaCadenas.add(cadena);
    	}
    ...
    public void insertarEntero( int entero, int posicion )
    {
        ...
        // Si la posición es mayor o igual al tamaño de la lista, insertar al final de la lista
        else if (posicion >= listaEnteros.size()) {
            listaEnteros.add(entero);
        }
        // Si la posición está dentro de los límites de la lista, insertar en la posición especificada
        else {
    ...
    public void eliminarEnteroPorPosicion( int posicion )
    {
    	...
    	if ( 0<= posicion && posicion<= listaEnteros.size()-1) {
    ...
    {
    	List<Integer> listicaNueva = new ArrayList<>();
    	
    	for (double valor: valores) {
    		int enteroMeter = (int) valor;
    		listicaNueva.add(enteroMeter);
    	}
    	
    	listaEnteros= listicaNueva;
    ...
    public void volverPositivos( )
    {
    	for (Integer entero: listaEnteros) {
    		if (entero <0) {
    			...
    			listaEnteros.set(indice, entero* -1);
    		}
    	}
    ...
    public int contarApariciones( String cadena )
    {
        ...
        for (String str : listaCadenas) {
            if (str.toLowerCase().equals(cadenaLowerCase)) {
                contador++;
            }
        }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (Integer entero : listaEnteros) {
            if (!numerosUnicos.contains(entero)) {
                numerosUnicos.add(entero);
            } else if (!numerosRepetidos.contains(entero)) {
                numerosRepetidos.add(entero);
                contador++;
            }
        }
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
        ...
        // Generar y agregar valores aleatorios a la lista
        for (int i = 0; i < cantidad; i++) {
            int valorAleatorio = (int) (Math.random() * (maximo - minimo + 1)) + minimo;
            enterosAleatorios.add(valorAleatorio);
        }

        // Reemplazar los elementos de la lista original con los nuevos valores generados
        listaEnteros.clear();
        listaEnteros.addAll(enterosAleatorios);
